...
def suggest_captain(
    limit: int = 5,
    consider_early_season: bool = None,
    points_weight: float = 0.4,
    form_weight: float = 0.3,
    attacking_weight: float = 0.15,
    fixture_weight: float = 0.10,
    ...
    - For early gameweeks (first 10): factor in previous season performance
    - Consider fixture difficulty where available
    - Prioritize premium players who justify captaincy

    Args:
        limit: Number of captain suggestions to return (default: 5)
        consider_early_season: Whether to factor previous season data for early gameweeks.
                              If None, will auto-detect based on current gameweek
        points_weight: Weight for total points factor (default: 0.4)
        form_weight: Weight for recent form factor (default: 0.3)
        attacking_weight: Weight for expected goal involvements (default: 0.15)
        fixture_weight: Weight for fixture difficulty factor (default: 0.10)
        history_weight: Weight for previous season bonus (default: 0.05)

    Returns:
        String with captain recommendations ranked by suitability

    Note: Weights should sum to 1.0 for proper scoring. Default weights are:
    Total Points (40%) + Form (30%) + Attacking (15%) + Fixtures (10%) + History (5%) = 100%
    """
    try:
        # Validate weights sum to approximately 1.0
        total_weight = points_weight + form_weight + attacking_weight + fixture_weight + history_weight
        if abs(total_weight - 1.0) > 0.01:
            return f"Warning: Weights sum to {total_weight:.3f}, should be close to 1.0 for proper scoring."
        ...
        captain_columns = ['id', 'web_name', 'first_name', 'second_name', 'team', 'element_type',
                          ...
                          'minutes', 'event_points', 'expected_goals', 'expected_assists',
                          'expected_goal_involvements', 'influence', 'creativity', 'threat', 'ict_index',
                          'avg_fixture_difficulty_3', 'avg_fixture_difficulty_5', 'goals_scored', 'assists']

        # Load only available columns
        all_elements = pd.read_parquet(elements_path)
        available_columns = [col for col in captain_columns if col in all_elements.columns]
        elements_df = pd.read_parquet(elements_path, columns=available_columns)
        ...
        numeric_cols = ['form', 'points_per_game', 'selected_by_percent', 'expected_goals',
                       ...
                       'creativity', 'threat', 'ict_index', 'avg_fixture_difficulty_3', 'avg_fixture_difficulty_5']
        ...
        # Add computed columns
        elements_df['team_name'] = elements_df['team'].map(team_lookup)
        position_lookup = {1: 'GK', 2: 'DEF', 3: 'MID', 4: 'FWD'}
        elements_df['position'] = elements_df['element_type'].map(position_lookup)
        elements_df['price_display'] = elements_df['now_cost'] / 10

        # Detect if we're in early season (first 10 gameweeks) - use total_points as proxy
        max_total_points = elements_df['total_points'].max()
        is_early_season = max_total_points < 100  # Rough heuristic: if max points < 100, likely early season

        if consider_early_season is None:
        ...
        if captain_candidates.empty:
            return "No suitable captain candidates found based on position, price and playing time criteria."

        # Calculate captain score using configurable weights
        captain_candidates['captain_score'] = 0.0

        # Factor 1: Total points - normalize to 0-100 scale
        max_points = captain_candidates['total_points'].max()
        if max_points > 0:
            captain_candidates['points_score'] = (captain_candidates['total_points'] / max_points) * 100
            captain_candidates['captain_score'] += captain_candidates['points_score'] * points_weight

        # Factor 2: Recent form
        if 'form' in captain_candidates.columns:
            captain_candidates['form'] = captain_candidates['form'].fillna(0)
            max_form = captain_candidates['form'].max()
            if max_form > 0:
                captain_candidates['form_score'] = (captain_candidates['form'] / max_form) * 100
                captain_candidates['captain_score'] += captain_candidates['form_score'] * form_weight

        # Factor 3: Expected goal involvements - attacking threat
        if 'expected_goal_involvements' in captain_candidates.columns:
            captain_candidates['expected_goal_involvements'] = captain_candidates['expected_goal_involvements'].fillna(0)
            max_xgi = captain_candidates['expected_goal_involvements'].max()
            if max_xgi > 0:
        ...
        if consider_early_season and history_weight > 0:
            try:
                ...
                if not history_df.empty:
                    recent_season = history_df['season_name'].max()
                    last_season = history_df[history_df['season_name'] == recent_season]

                    # Create lookup for last season points per game
                    last_season_ppg = {}
                    for _, player in last_season.iterrows():
                        if player['minutes'] > 1000:  # Only if they played regularly
                            ppg = player['total_points'] / 38  # Assume 38 gameweeks
                            last_season_ppg[player['player_id']] = ppg

                    # Apply bonus for players who performed well last season
                    captain_candidates['last_season_bonus'] = captain_candidates['id'].map(last_season_ppg).fillna(0)
                    max_last_season = captain_candidates['last_season_bonus'].max()
                    if max_last_season > 0:
                        captain_candidates['last_season_score'] = (captain_candidates['last_season_bonus'] / max_last_season) * 100
                        captain_candidates['captain_score'] += captain_candidates['last_season_score'] * history_weight
            except:
        ...
        captain_data = {
            ...
            'Form (5GW)': top_captains.get('form', 0).fillna(0).round(1),
        ...
        # Add history score column if applicable
        if consider_early_season and history_weight > 0:
            captain_data[f'History ({history_weight:.0%})'] = top_captains.get('last_season_score', 0).fillna(0).round(1)

        captain_df = pd.DataFrame(captain_data)

        # Create result with header info and table
        result = f"""🎯 **Captain Suggestions for This Gameweek**

📊 **Scoring Formula:** Points({points_weight:.0%}) + Form({form_weight:.0%}) + Attack({attacking_weight:.0%}) + Fixtures({fixture_weight:.0%}) + History({history_weight:.0%})"""

        if consider_early_season:
            result += "\n📈 *Early season analysis includes previous season performance*"

        result += f"\n\n## Captain Analysis\n\n{captain_df.to_markdown(index=False)}"

        result += f"""

### Factor Breakdown
- **Final Score**: Weighted combination of all factors (0-100 scale)
...
- **Own%**: Current ownership percentage"""
        ...
        error_msg = f"Error suggesting captains: {str(e)}"
