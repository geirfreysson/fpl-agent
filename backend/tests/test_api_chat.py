...
def test_chat_endpoint_streaming_player():
    """Test that the chat endpoint returns a proper streaming response."""
    payload = {
        "message": [{"type": "text", "text": "What can you tell me about Saka?"}],
    ...
    # Print the final text for debugging to see markdown formatting
    print(f"\n--- FINAL ANSWER ---\n{final_text}\n--- END FINAL ANSWER ---\n")
    ...
    # Check that it contains some markdown formatting
    assert len(final_text) > 50, f"Final answer should be substantial but got: {final_text}"


def test_chat_endpoint_streaming():
    """Test that the chat endpoint returns a proper streaming response."""
    payload = {
        "message": [{"type": "text", "text": "What 10 players have the highest total scores?"}],
        "conversation_history": [],
        "context": {}
    }
...
def test_chat_endpoint_suggest_captain():
    """Test that the suggest_captain tool works properly."""
    payload = {
        "message": [{"type": "text", "text": "who should i captain this gameweek"}],
        "conversation_history": [],
        "context": {}
    }
    ...
    # Check that captain suggestions are provided
    assert len(final_text) > 50, f"Captain suggestion should be substantial but got: {final_text}"
